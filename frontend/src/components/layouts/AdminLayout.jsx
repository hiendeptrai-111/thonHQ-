...
import { LayoutDashboard, Users, FileText, Settings, LogOut, MessageSquare, Bell, User, Vote, MessageCircle, Lightbulb } from 'lucide-react';
...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex font-sans">
      {/* Sidebar - Modern Youth Union themed deep blue */}
      <aside className="w-72 bg-gradient-to-b from-blue-900 via-slate-900 to-slate-900 text-white flex flex-col hidden md:flex fixed h-full z-10 shadow-xl border-r border-white/5">
        <div className="h-20 flex items-center px-6 border-b border-white/5 space-x-3 bg-blue-950/20">
      ...
      <div className="flex-1 md:ml-72 flex flex-col min-h-screen">
        {/* Modern Top Header */}
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-10 transition-all">
          <div className="flex flex-col">
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">
              {navItems.find(item => item.path === location.pathname)?.name || 'Bảng điều khiển'}
            </h1>
            <p className="text-xs text-slate-400 font-semibold mt-0.5">Trang chủ / Admin Portal</p>
          </div>
